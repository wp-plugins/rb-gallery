...
function wprb_gallery_options_page() {
		...
		if( isset( $_POST['wprbgall_form_submitted'] ) ) {
			...
			if ($hidden_field == 'Y' && !empty($_POST['wprbgall_albumname']) ) {
				...
				$wprbgall_albumorder = esc_html( $_POST['wprbgall_albumorder'] );
				$rb_gal_table = rb_gallery_albums();

				//input name into database
				$wpdb->insert($rb_gal_table, array("album_name" => $wprbgall_albumname, "album_date" => date('Y-m-d'), "description" => $wprbgall_albumdesc, "album_order" => $wprbgall_albumorder ));
			...
			if ($hidden_field == 'Y' && !empty($_POST['wprbgall_album_id']) ) {
				...
				$wpdb->update( 
					...
					array(
						...
						"album_order" => $wprbgall_albumorder 
						), 
...
function plugin_install_script_for_wprb_gallery()
{
    ...
    $rb_gallery = ABSPATH . 'wp-content/rb-gallery/';
    $album_thumbs = ABSPATH . 'wp-content/rb-gallery/album-thumbs';
    $dontremove = ABSPATH . 'wp-content/rb-gallery/dontremove';
    $gallery_uploads = ABSPATH . 'wp-content/rb-gallery/gallery-uploads';
    ...
    $new_loc = ABSPATH . 'wp-content/rb-gallery/dontremove/dontremove.jpg';
